...
const SnApproveDetail = ({ dispatch, match, snApprove }) => {
    ...
    const handleVideo = (url) => {
        ...
        if (urlIndex) {
            return detail.inspectionValue.split(',').map((item, index) => <div key={index}>{item}</div>)
        } else {
            return url
        }
    }
            ...
            <PageHeaderWrapper className='mains' style={{ paddingBottom: '20px' }}>
                <div className='descriptions-layout'>
                    <Descriptions title="" column={1}>
                        ...
                        <Descriptions.Item label="查验方式">{detail.inspectionType}</Descriptions.Item>
                        {
                            detail.inspectionType == '现场查验' && (<Descriptions.Item label="查验审批单">
                ...
                }
                {
                    ...
                    (
                        <>
                            ...
                            <Divider></Divider>
                            <Table rowKey={"id"} columns={columns} pagination={false}  dataSource={detail.logs}></Table>
                        </>
                    )
                }
            </PageHeaderWrapper>
